...
class AutoTable:
    ...
    def __init__(self, fname=None, compression_level=1):
        ...
        :type  fname: string
        :param compression_level: Specifies the compression level used when storing data. (default: 1)
        :type  compression_level: int

        If fname is not specified (or is None), fname will be derived from sys.argv[0] by striping the extension 
        and adding ".h5".  As a result, the file will be named like the creating program.  
    ...
    def _create_table(self, name, example):
        ...
        if type(example) == np.ndarray:
            ...
            self.tables[name] = h5.create_earray(path, nmpt[-1], h5type, h5dim,
                                                filters=filters)
            ...
            self.tables[name] = h5.create_vlarray(path, nmpt[-1], h5type,
                                                filters=filters)
